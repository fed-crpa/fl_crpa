...
experimentNamesMNIST = [
    "2024_12_03_12:57:23_MNIST_dp_comparison",
    "2024_12_03_12:57:23_MNIST_dp_comparison",
    "2024_12_03_12:57:23_MNIST_dp_comparison",
]

experimentNamesFMNIST = [
    "2024_12_03_12:57:23_FMNIST_dp_comparison",
    "2024_12_03_12:57:23_FMNIST_dp_comparison",
    "2024_12_03_12:57:23_FMNIST_dp_comparison",
]
...
def generate_df_for_experiments(experimentNames):
    ...
    for experimentName in experimentNames:
        ...
        for runID in runsPandas["run_id"]:
            ...
            newRowDF = pd.DataFrame.from_records(
                [
                    {
                        "experimentName": experimentName,
                        "combination": combination,
                        "alpha": data_distributions,
                        "avg attack accuracy": avg,
                    }
                ]
            )
    ...
    resultMetrics = (
        dfComplete.drop(["experimentName"], axis=1)
        .groupby(by=["combination", "alpha"])
        .agg(["min", "max", "mean", "std", lambda x: max(x) - min(x)])
    )
    ...
    resultMetrics.columns = [
        "_".join(pair) if "" not in pair else pair[0] for pair in resultMetrics.columns
    ]
    print(resultMetrics)

    seaplot = sns.catplot(
        x="alpha",
        y="avg attack accuracy_mean",
        hue="combination",
        hue_order=hue_order,
        data=resultMetrics,
        kind="bar",
    ...
    plt.title(name)
    plt.savefig("differential_privacy_" + name + ".png", bbox_inches="tight")
