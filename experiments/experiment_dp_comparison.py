...
for DATASET in ["MNIST","FMNIST"]:
        for stat_rep in range(3):
                experiment_name=datetime.strftime(datetime.now(), "%Y_%m_%d_%H:%M:%S")+"_"+DATASET+"_dp_comparison"
                ...
                for pair in pairs:
                        for distribution in distributions:
                                ...
                                GENERATOR_ATTACK, experiment_name, run_name, l2_norm_clip, noise_multiplier, delta,
                                distributionFolder, POISON_ATTACK, GPU, NUM_TO_POISON, adam_for_non_private_training, sgd_for_dp_training, global_lr))
                                proc.start()
                                proc.join()
